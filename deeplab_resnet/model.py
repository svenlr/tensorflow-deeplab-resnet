...
class DeepLabResNetModel(Network):
    def setup(self, is_training, num_classes, is_inference, add_extra_head):
        ...
        stock_head_classes = 7 if add_extra_head else num_classes

        (self.feed('res5b_relu',
                   'bn5c_branch2c')
         .add(name='res5c')
         .relu(name='res5c_relu')
         .atrous_conv(3, 3, stock_head_classes, 6, padding='SAME', relu=False, name='fc1_voc12_c0'))

        (self.feed('res5c_relu')
         .atrous_conv(3, 3, stock_head_classes, 12, padding='SAME', relu=False, name='fc1_voc12_c1'))

        (self.feed('res5c_relu')
         .atrous_conv(3, 3, stock_head_classes, 18, padding='SAME', relu=False, name='fc1_voc12_c2'))

        (self.feed('res5c_relu')
         .atrous_conv(3, 3, stock_head_classes, 24, padding='SAME', relu=False, name='fc1_voc12_c3'))
        ...
        if add_extra_head:
            ...
            (self.feed('res5c_relu',
                       'fc1_voc12_stock')
             .concat(name='extra_head_input', axis=3))

            # second head for custom classes
            (self.feed('extra_head_input')
